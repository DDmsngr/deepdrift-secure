...
class _HomeScreenState extends State<HomeScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      // Сообщаем сокету что уходим в фон — он НЕ будет пытаться переподключиться
      // пока в фоне (иначе auth_challenge зависает без ответа → бесконечный цикл)
      _socket.onAppPaused();
    ...
    if (state == AppLifecycleState.resumed) {
      // Всегда форс-переподключаемся — forceReconnect() сбрасывает _isConnecting
      // и _isInBackground, даже если фоновая попытка зависла
  ...
  Future<String?> _uploadAvatarWithToken(File file) async {
    try {
      ...
      if (resp.statusCode == 200 && resp.data['status'] == 'success') {
        return resp.data['file_id'] as String?;
      }
    } catch (e) {
                ...
                children: [
                  GestureDetector(
                    onTap: () async {
                      ...
                      if (img != null) {
                        final fileId = await _uploadAvatarWithToken(File(img.path));
