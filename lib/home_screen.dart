...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _autoConnect() async {
    try {
      ...
      await _cipher.init(savedPassword, savedSalt,
  ...
  Future<void> _registerPublicKeysOnServer() async {
    ...
    } catch (e) { print('Failed to register public keys: $e'); }
  }
  ...
  Future<void> _handleIncomingMessageQuietly(Map<String, dynamic> data) async {
    ...
    try {
      final decrypted = await _cipher.decryptText(data['encrypted_text'], fromUid: senderUid);
      final msg = {
    ...
    showDialog(
      ...
      builder: (c) => StatefulBuilder(
        builder: (context, setDialogState) {
          bool hasAvatar = currentAvatar != null &&
              ...
              ElevatedButton(
                onPressed: () async {
                  await _storage.saveMyProfile(
                      nickname: nameCtrl.text.trim(), avatarUrl: currentAvatar);
  ...
  void _addContactWithId(String contactId) {
    ...
    Navigator.push(context, MaterialPageRoute(
            builder: (c) => ChatScreen(myUid: _myUid!, targetUid: contactId, cipher: _cipher)))
    ...
    return ListView.builder(
      ...
      itemBuilder: (c, i) {
        ...
        return ListTile(
          leading: Stack(children:[
            CircleAvatar(
              ...
              backgroundImage: hasAvatar
                  ...
                  : null,
              child: !hasAvatar
                  ? Text(name[0].toUpperCase(), style: const TextStyle(color: Colors.cyan))
                  : null,
            ),
            if (isOnline)
          ...
          onTap: () {
            Navigator.push(context, MaterialPageRoute(
                    builder: (c) => ChatScreen(myUid: _myUid!, targetUid: uid, cipher: _cipher)))
                .then((_) => setState(() => _chats = _storage.getContactsSortedByActivity()));
          },
  ...
  Widget build(BuildContext context) {
    ...
    bool hasMyAvatar  = myProfile['avatarUrl'] != null &&
        myProfile['avatarUrl']!.isNotEmpty && myProfile['avatarUrl'] != 'null';
    ...
    return PopScope(
      ...
      child: Scaffold(
              ...
              IconButton(
                ...
                onPressed: () => Navigator.push(context, MaterialPageRoute(
                    builder: (_) => SettingsScreen(
                        storage: _storage, cipher: _cipher, myUid: _myUid ?? ''))),
              ),
              GestureDetector(
            ...
            child: Row(children:[
              ...
              Expanded(
                child: TextField(
                  ...
                  onSubmitted: (value) {
                    if (value.isNotEmpty) { _addContactWithId(value); _quickIdController.clear(); }
                  },
                ),
              ),
              const SizedBox(width: 8),
        ...
        floatingActionButton: _isSearching ? null : FloatingActionButton(
            onPressed: _addContact, backgroundColor: Colors.cyan,
  ...
  Widget _buildSearchResults() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (c, i) {
        ...
        return ListTile(
          leading: CircleAvatar(backgroundColor: const Color(0xFF1A1F3C),
              child: Text(name[0].toUpperCase(), style: const TextStyle(color: Colors.cyan))),
          title: Text(name),
          ...
          onTap: () {
            ...
            Navigator.push(context, MaterialPageRoute(
                    builder: (c) => ChatScreen(myUid: _myUid!, targetUid: r['chatWith'], cipher: _cipher)))
                .then((_) => setState(() => _chats = _storage.getContactsSortedByActivity()));
          },
        );
      },
